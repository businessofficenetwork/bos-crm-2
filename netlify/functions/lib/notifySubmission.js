// Emails (and texts) Keri whenever a new claim is submitted through the
// portal (or, later, the email intake path) - so she knows something
// landed without having to keep the CRM open. Best-effort, same pattern
// as the other notify* modules: a failed/skipped send never blocks the
// submission itself.
...
export async function notifySubmission({ contractorName, address, missing }) {
  ...
  await sendText(`${subject} ${CRM_ORIGIN}/audits`)
